...
class PCAFeatureSelector(BaseEstimator, TransformerMixin):
    """
    Selects features using PCA dimensionality reduction.
    ...
    def fit(self, X, y=None):
        ...
        self.num_components = np.argmax(explained_variance >= self.variance_threshold) + 1
        return self
    ...
    def transform(self, X):
        ...
        return pd.concat([X[self.column_config['protected_cols']], X_pca_df], axis=1)
